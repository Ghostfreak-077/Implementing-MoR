...
from models.mor_models import LinearRouter
...
class MoRLlamaDecoderLayer(nn.Module):
    """The Mixtures of Depth Block that dynamically assigns which tokens to process in a block.
    ...
    def __init__(self, config, block, cfg, capacity_factor=1.0, cap_warmup_step=0,):
        ...
        if not cfg.mor.rand_router:
            self.mor_router = LinearRouter(config).to(torch_dtype)

        if cfg.mor.expert.sampling == "aux_router":
            self.mlp_router = LinearRouter(config).to(torch_dtype)
